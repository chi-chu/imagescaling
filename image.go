...
func New(in io.Reader)(*Image, error) {
    ...
    o.ext = fm
    o.image = origin
    if img, ok := origin.(*image.Paletted); ok {
        o.palette = make([]color.NRGBA, len(img.Palette))
        for i := 0; i < len(img.Palette); i++ {
            o.palette[i] = color.NRGBAModel.Convert(img.Palette[i]).(color.NRGBA)
        }
    }
    return &o, nil
}
...
func (m *Image) adjust(x1, y1, x2, y2 int, dst []uint8){
    if m.opImage == nil {
        m.opImage = m.image
    }
    switch img := m.opImage.(type) {
    case *image.NRGBA:
        size := (x2 - x1) * 4
        j := 0
    ...
    default:
        ...
        for y := y1; y < y2; y++ {
            for x := x1; x < x2; x++ {
                r16, g16, b16, a16 := m.tempImage.At(x, y).RGBA()
                d := dst[j : j+4 : j+4]
                switch a16 {
                case 0xffff:
                    d[0] = uint8(r16 >> 8)
